import argparse
import json
from trainers import Trainer, TrainerAdaptive
from evaluators import Evaluator, EvaluatorAllAttacks
...
def main(args):
    ...
    if configs.mode == 'train':
        ...
        trainer.train()
    elif configs.mode == 'train_adaptive':
        configs.gamma = float(configs.gamma) / 255
        trainer = TrainerAdaptive(configs)
        trainer.train()
    elif configs.mode == 'eval':
        evaluator = Evaluator(configs)
        evaluator.eval()
    elif configs.mode == 'eval_all':
        evaluator = EvaluatorAllAttacks(configs)
        evaluator.eval()
